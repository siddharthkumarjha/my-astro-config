  ...
  specs = {
    {
      ...
      opts = function(_, opts)
        ...
        maps.t["<M-3>"] = { "<Cmd>ToggleTerm direction=float<CR>", desc = "Toggle terminal" } -- requires terminal that supports binding <C-'>
        maps.i["<M-3>"] = { "<Esc><Cmd>ToggleTerm direction=float<CR>", desc = "Toggle terminal" } -- requires terminal that supports binding <C-'>
      end,
    },
  },
  opts = {
    ...
    on_create = function(t)
      ...
      if t.hidden then
        ...
        vim.keymap.set({ "n", "t", "i" }, "<M-3>", toggle, { desc = "Toggle terminal", buffer = t.bufnr })
